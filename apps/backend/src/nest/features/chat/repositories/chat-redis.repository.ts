...
export class ChatRedisRepository extends EntityRepository<Chat> {
    ...
    async acceptChatRequest(chatId: string): Promise<ChatDTO> {
        const chat = await this.findOne({ where: 'chatId', eq: chatId });
        if (!chat) return;
    ...
    async getChats({ offset = 0, count = 25 }: { offset?: number; count?: number } = {}): Promise<ChatDTO[]> {
        try {
            const chats = await this.findAll({ offset, count });
    ...
    async getAcceptedChats({ offset = 0, count = 25 }: { offset?: number; count?: number } = {}): Promise<ChatDTO[]> {
        try {
            const chats = await this.findAllWhereEq({ offset, count, where: 'acceptedChat', eq: true });
            return chats.map(chat => chat.toJSON());
        } catch (error) {
    ...
    async deleteChat(chatId: string): Promise<void> {
        ...
        try {
            return await this.delete(chatToDelete.entityId);
        } catch (error) {
